$(document).ready(() => {
	$(window).scroll(() => {
		...
		} else if (this.scrollY > 2 * window.innerHeight && this.scrollY < 4.3 * window.innerHeight) {
			$("meta[name='theme-color']").attr("content", "#ffd1dc");
		} else if (this.scrollY > 4.3 * window.innerHeight && this.scrollY < 5.3 * window.innerHeight) {
			$("meta[name='theme-color']").attr("content", "#add8e6");
		}
		if(this.scrollY>2.0*window.innerHeight){
			$(".animated-heart").css("position","fixed");
			let scale = (this.scrollY - 2.0*window.innerHeight)*20/(window.innerHeight);
			let opacity = (this.scrollY -2.0*window.innerHeight)/(window.innerHeight);
			if(scale>1)$(".animated-heart").css("transform","scale("+scale+")");
			if(scale>1)$(".animated-heart").css("-webkit-transform","scale("+scale+")");
			if(scale>1)$(".animated-heart").css("-ms-transform","scale("+scale+")");
